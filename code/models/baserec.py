...
class BaseRecommender(ABC, nn.Module):
    ...
    def recommend(
        ...
        for user_id_batch in test_user_batches:
            ...
            if hash_type == 'spectral_label_overlap':
                user_id_hashed = user_id_batch
            ...
            for user in user_id_batch:
                # Coping with cold start situations
                candidates = train_candidates.get(user, [])
                mask_user_idx.extend([user_id_batch_2_idx[user]] * len(candidates))
                mask_item_idx.extend(candidates)
    ...
    def recommend_list(
        ...
        top_k_indices_all = {kk: [] for kk in k_list}
        ...
        for user_id_batch in test_user_batches:
            ...
            if hash_type == 'spectral_label_overlap':
                user_id_hashed = user_id_batch
                all_item_id_hashed = np.arange(len(item_clusters))
                scores = self.get_scores(
                    hash_type, device, user_id_hashed, all_item_id_hashed
                )
            else:
            ...
            scores[mask_user_idx, mask_item_idx] = -1e4
            max_k = k_list[-1]
            _, top_k_indices = torch.topk(scores, max_k, dim=1)
            for kk in k_list:
                top_k_indices_all[kk].append(top_k_indices[:, :kk])
            # top_k_indices_all.append(top_k_indices)

        # top_k_indices_all = torch.cat(top_k_indices_all, dim=0)
        for kk in k_list:
            top_k_indices_all[kk] = torch.cat(top_k_indices_all[kk], dim=0)

...
class CustomLouvain(Louvain):
    def _pre_processing(self, input_matrix, force_bipartite):
        ...
        if self.modularity == 'potts':
            ...
            raise ValueError(f'CustomLouvain: Unknown modularity function {self.modularity}.')
...
def Louvain_hash(
    ...
    lv = CustomLouvain(resolution=resolution, modularity=mod_type)
    # lv = Louvain(resolution=resolution, modularity=mod_type)
    lv.fit(biadjacency, force_bipartite=True)
    if Secondary_Clusters == False:
        time_cost = time.time() - time_begin
        user_clusters_co = lv.labels_row_
        item_clusters_co = lv.labels_col_
        ...
        labels = np.concatenate([lv.labels_row_, lv.labels_col_])
        ...
        for x in range(num_user):
            ...
            for label_target, now_value in prob_labels.items():
                now_value = now_value - prob_labels.get(label_now,0) - resolution * deg[x] * (cluster_sum_I[label_target] - cluster_sum_I[label_now])
